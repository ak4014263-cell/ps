...
router.get('/', async (req, res) => {
  try {
    ...
    let sql = `
      SELECT p.*, c.client_name, c.company, c.email as client_email
      FROM projects p
    ...
    if (status && status !== 'All') {
      where.push('p.status = ?');
      params.push(status.toLowerCase().replace(/ /g, '_'));
    }

    if (keyword) {
      where.push('(p.project_name LIKE ? OR p.project_number LIKE ? OR p.description LIKE ?)');
      params.push(`%${keyword}%`);
      params.push(`%${keyword}%`);
      params.push(`%${keyword}%`);
    }
    ...
    const transformed = projects.map(p => ({
      ...p,
      name: p.project_name, // Map project_name to name for frontend compatibility
      client: p.client_id ? {
