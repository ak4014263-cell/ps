...
router.get('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const templates = await getAll('SELECT * FROM templates WHERE id = ?', [id]);
...
router.post('/generate-pdf', async (req, res) => {
  const { templateData, records, options, batchIndex } = req.body;
  ...
  try {
    ...
    const dataFile = path.join(tempDir, `data_${tempId}.json`);
    const outputFile = path.join(__dirname, '..', 'uploads', 'pdfs', `batch_${tempId}.pdf`);
    const pdfDir = path.dirname(outputFile);
    ...
    fs.writeFileSync(dataFile, JSON.stringify(pythonData));

    const pythonScript = path.join(__dirname, '..', 'tools', 'template_pdf_generator.py');

    const { spawn } = await import('child_process');
    const python = spawn('python', [pythonScript, dataFile, outputFile]);
    ...
    python.on('close', (code) => {
      ...
      if (code !== 0) {
        console.error('Python PDF generator error:', errorData);
        return res.status(500).json({ error: 'PDF generation failed', detail: errorData });
      }

      const pdfUrl = `http://localhost:3001/uploads/pdfs/batch_${tempId}.pdf`;
      res.json({ url: pdfUrl });
    });
