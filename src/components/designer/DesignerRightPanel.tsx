...
import { Settings, Layers, LayoutTemplate, Images, HelpCircle, Keyboard, PanelRight, PanelRightClose, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function DesignerRightPanel({
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Button
            variant="ghost"
            size="icon"
            className="h-9 w-9 text-muted-foreground hover:text-foreground hover:bg-muted mb-2"
            onClick={() => setIsCollapsed(!isCollapsed)}
          >
            {isCollapsed ? <ChevronLeft className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
          </Button>
        </TooltipTrigger>
        <TooltipContent side="left" className="bg-popover text-popover-foreground border-border z-50">
  ...
  const panelContent = (
    <div className={`flex h-full bg-background border-l border-border transition-all duration-200`}>
      {/* Content Area - hidden when collapsed */}
      {!isCollapsed && (
        <div className="w-64 lg:w-72 xl:w-[300px] min-w-0 overflow-hidden h-full">
