...
export function DesignerPropertiesPanel({
  ...
  const updateProperty = (key: string, value: any) => {
    ...
    if (key === 'width') {
      ...
      try {
        if (typeof (selectedObject as any)._clearCache === 'function') {
          (selectedObject as any)._clearCache();
        }
        if (typeof selectedObject.initDimensions === 'function') {
          selectedObject.initDimensions();
        }
        if (typeof selectedObject.setCoords === 'function') {
        ...
        <div className="space-y-1">
            ...
            <Section title="Typography" icon={<Type className="h-3.5 w-3.5" />} defaultOpen={true}>
              ...
              <div className="space-y-1">
                ...
                <div className="flex gap-1">
                  <Button
                    ...
                    onClick={() => {
                      updateProperty('lineHeight', 1.0);
                      updateProperty('charSpacing', 0);
                    }}
                  >
                    Compact
                  </Button>
                  ...
                  <Button
                    ...
                    className="flex-1 h-7 text-xs"
                    onClick={() => {
                      updateProperty('lineHeight', 1.6);
                      updateProperty('charSpacing', 20);
                    }}
                  >
                    Relaxed
                  </Button>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">{properties.lineHeight.toFixed(1)}</span>
                </div>
                <Slider
                  value={[properties.lineHeight]}
                  onValueChange={([v]) => updateProperty('lineHeight', v)}
                  min={0.5}
                  max={4}
                  step={0.1}
                  className="py-2"
          ...
          {isPhotoObject && (
            <Section title="Photo Settings" icon={<ImageIcon className="h-3.5 w-3.5" />} defaultOpen={true}>
              <div className="space-y-3">
                {/* Photo Preview from Database */}
                {selectedObject?.data?.previewUrl && (
                  <div className="space-y-1">
