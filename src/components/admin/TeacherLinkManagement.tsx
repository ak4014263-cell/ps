...
export function TeacherLinkManagement() {
  ...
  const { data: links = [], isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await apiService.teacherLinksAPI.getAll();
        return (response.data || response || []);
      } catch (error) {
        console.error('Failed to fetch teacher links:', error);
        return [];
      }
    },
  });
  ...
  const { data: submissions = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const response = await apiService.teacherLinksAPI.getSubmissions(selectedLinkId);
        return (response.data || response || []);
      } catch (error) {
  ...
  const createLinkMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!newLink.teacherName) {
        throw new Error('Teacher name is required');
      }

      const response = await apiService.teacherLinksAPI.create({
  ...
  const toggleActiveMutation = useMutation({
    mutationFn: async ({ id, isActive }: { id: string; isActive: boolean }) => {
      return apiService.teacherLinksAPI.update(id, { is_active: !isActive });
    },
    onSuccess: () => {
  ...
  const deleteLinkMutation = useMutation({
    mutationFn: async (id: string) => {
      return apiService.teacherLinksAPI.delete(id);
    },
    onSuccess: () => {
