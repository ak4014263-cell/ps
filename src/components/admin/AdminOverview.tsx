...
export function AdminOverview() {
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        const [vendorsRes, clientsRes, projectsRes, templatesRes] = await Promise.all([
        ...
        const publicTemplates = templates.length;

        const projectsByStatus = {
          draft: projects.filter((p: any) => p.status === 'draft').length,
          data_upload: projects.filter((p: any) => p.status === 'data_upload').length,
          design: projects.filter((p: any) => p.status === 'design').length,
          printing: projects.filter((p: any) => p.status === 'printing').length,
