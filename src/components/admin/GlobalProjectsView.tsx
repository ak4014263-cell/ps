...
export function GlobalProjectsView() {
  ...
  const { data: projects = [], isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const result = await apiService.projectsAPI.getAll();
      let filteredProjects = result.data || [];

      if (selectedVendor !== 'all') {
        filteredProjects = filteredProjects.filter((p: any) => p.vendor_id === selectedVendor);
      }

      if (selectedStatus !== 'all') {
        filteredProjects = filteredProjects.filter((p: any) => p.status === selectedStatus);
      }
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ projectId, status }: { projectId: string; status: ProjectStatus }) => {
      await apiService.projectsAPI.update(projectId, { status });
    },
    onSuccess: () => {
  ...
  const reassignProjectMutation = useMutation({
    mutationFn: async ({ projectId, vendorId }: { projectId: string; vendorId: string }) => {
      await apiService.projectsAPI.update(projectId, { vendor_id: vendorId });
    },
    onSuccess: () => {
