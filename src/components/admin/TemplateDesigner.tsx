...
export function TemplateDesigner({ onSave, editTemplate }: TemplateDesignerProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editTemplate?.id) {
        const result = await apiService.templatesAPI.update(editTemplate.id, templateData);
        if (!result.success) throw new Error(result.error || 'Failed to update template');
      } else {
        const result = await apiService.templatesAPI.create(templateData);
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 min-h-[600px]">
      ...
      <div className="flex-1 flex flex-col items-center">
        ...
        <div className="border-2 border-dashed border-muted-foreground/30 rounded-lg p-8 bg-muted/20 overflow-auto min-h-[500px] flex items-center justify-center" style={{ overflowX: 'auto', overflowY: 'auto' }}>
          <div className="flex items-center justify-center">
            <div style={{ display: activeSide === 'front' ? 'flex' : 'none', alignItems: 'center', justifyContent: 'center' }}>
              <canvas ref={canvasRef} className="shadow-lg rounded border border-muted-foreground/20" />
            </div>
            {hasBackSide && (
              <div style={{ display: activeSide === 'back' ? 'flex' : 'none', alignItems: 'center', justifyContent: 'center' }}>
                <canvas ref={backCanvasRef} className="shadow-lg rounded border border-muted-foreground/20" />
              </div>
            )}
          </div>
        </div>
