...
export function CreateStaffForm() {
  ...
  // Get vendors (admin view) or use current vendor (vendor view)
  ...
  const { data: vendors = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!isAdmin) {
        // For vendor users, just return their own vendor
        return [];
      }
      try {
  ...
  const form = useForm<StaffFormValues>({
    ...
    defaultValues: {
      ...
      phone: '',
      role: 'vendor_staff',
      vendorId: isAdmin ? '' : (user?.vendor_id || user?.id || ''),
    },
  });
  ...
  const onSubmit = async (values: StaffFormValues) => {
    ...
    try {
      const staffData = {
        ...
        role: values.role,
        vendorId: values.vendorId || (user?.vendor_id || user?.id) || null,
      };
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{isAdmin ? 'Assign to Vendor' : 'Vendor'}</FormLabel>
                    {isAdmin ? (
                      ...
                      <div className="text-sm text-muted-foreground p-2 bg-muted rounded">
                        {user?.vendor_id || user?.id} - Auto-assigned to your vendor
                      </div>
                    )}
                    <FormMessage />
                  </FormItem>
