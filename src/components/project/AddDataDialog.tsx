...
export function AddDataDialog({ projectId, groupId, triggerRef }: AddDataDialogProps) {
  ...
  const handleSaveData = async () => {
    ...
    try {
      // Get max record number for the project
      const maxRecordResponse = await apiService.dataRecordsAPI.getMaxRecordNumber(projectId);
      const maxRecordNumber = maxRecordResponse?.data?.max_record_number || 0;
      const startingNumber = maxRecordNumber + 1;
      ...
      for (let i = 0; i < records.length; i += batchSize) {
        ...
        await apiService.dataRecordsAPI.createBatch(batch);
      }
      ...
      toast.error(error?.message || 'Failed to add data records');
    } finally {
